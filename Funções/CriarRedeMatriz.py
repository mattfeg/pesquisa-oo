...
def CriarRedeMatriz(
    caminho_csv,
):  # Informar o caminho no seguinte formato './DadosSUS/CSV/xxxx.csv'
    ...
    dictDfs = {
        "Completa": dftotal,
        "Internação Domiciliar": dfInternaçaoDomiciliar,
        "Tratamento Clínico": dfTratamentoClinico,
        "Intercorrência": dfIntercorrencia,
        "Cirurgia": dfCirurgia,
    }
    ...
    for nome, dataf in dictDfs.items():
        ...
        tabela_cruzada.to_csv(
            f"./RedesMatrizFiltradas/{nome}/Matriz{caminho_csv.split('/')[-1].split('.')[0]}.csv",
            index=True,
            header=True,
        )
