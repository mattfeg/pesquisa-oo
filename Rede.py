            ...
            for hospital in df.index:
                if df[municipio][hospital] > 0:  # Remover arestas com peso 0
                    for index, row in self.__dfdistancia.iterrows():
                        if municipio == row['Source'] and hospital == row['Target'] and row['Weight'] > 0:
                            self.__rede.add_edge(municipio, hospital, weight=df[municipio][hospital],distancia=row['Distancia'],duracao=row['Duracao'])
