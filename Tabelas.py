...
# Adicionar os valores nas linhas
dfTabelaRedes.loc[1] = ['Completa', len(dftotal.index),'{:.2f}'.format(100*len(dftotal.index)/len(dftotal.index))+'%']
dfTabelaRedes.loc[2] = ['Tratamento Clínico', len(dfTratamentoClinico.index), '{:.2f}'.format(100*len(dfTratamentoClinico.index)/len(dftotal.index))+'%']
dfTabelaRedes.loc[3] = ['Intercorrência', len(dfIntercorrencia.index) ,'{:.2f}'.format(100*len(dfIntercorrencia.index)/len(dftotal.index))+'%' ]
dfTabelaRedes.loc[4] = ['Cirurgia', len(dfCirurgia.index), '{:.2f}'.format(100*len(dfCirurgia.index)/len(dftotal.index))+'%' ]
dfTabelaRedes.loc[5] = ['Internação Domiciliar', len(dfInternaçaoDomiciliar.index),'{:.2f}'.format(100*len(dfInternaçaoDomiciliar.index)/len(dftotal.index))+'%']
